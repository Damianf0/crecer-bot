@extends('layouts.app')
@section('title', 'Legajo · ' . $contacto->nombre)
...
/* Mini-player de audio embebido en el card (custom, no usamos <audio controls> nativo
   porque dentro del thumb con flex+overflow se renderiza inconsistente entre browsers). */
...
/* Audio: el wrapper centra. No aplicamos width:100%/height:100% al <audio> porque el flex lo achica. */
.lg-modal-foot {
    ...
    <div class="lg-head">
        ...
        <div class="lg-head-info">
            ...
            <div class="lg-head-meta">
                ...
                · <a href="/contactos" style="color:var(--info);">← Volver al directorio</a>
            </div>
        </div>
...
function cardHtml(d) {
    ...
    // Para audios usamos un mini-player custom (más robusto que <audio controls> en este layout).
    ...
    // El click en el card abre preview, salvo en audio: ahí dejamos que el usuario interactúe con los controles.
...
async function descargarZip() {
    ...
    // Forzar descarga via form post + window.open no funciona, usamos fetch + blob
    try {
    ...
    } catch { alert('No se pudo generar el ZIP.'); }
}
...
async function reenviarSeleccionados() {
    if (!_seleccionados.size) return;
    if (!CONTACTO_TIENE_WA) { alert('El contacto no tiene WhatsApp resuelto.'); return; }
    ...
    for (const id of _seleccionados) {
        ok + fail >= 0 && (btn.textContent = `Enviando… (${ok + fail + 1}/${total})`);
        try {
        ...
        // Pequeña pausa entre envíos para evitar rate-limit del bot/WA
...
async function prevReenviar() {
    ...
    const d = await r.json();
    if (d.ok) { cerrarPreview(); alert('Documento re-enviado por WhatsApp.'); }
    else alert(d.error || 'No se pudo re-enviar.');
}
...
// Cada card de audio tiene un único Audio() asociado por id, creado on-demand al primer ▶.
// Solo un audio reproduce a la vez: al iniciar uno se pausa el anterior.
const _mp = {};      // id → { audio, btn, fill, time, playing }
let _mpActivo = null;
